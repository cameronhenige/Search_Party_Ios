            ...
            if(user.searches != nil) {
                ...
                for search in user.searches ?? []{
                    if(!search.path.isEmpty) {
                        ...
                        if(selectedDay == nil) {
                            testline.color = user.color ?? "#808080"
                            
                        } else {
                            
                            if let selected = self.selectedDay, let searchCreated = search.created?.dateValue() {
                                if(isSameDay(date1: selected, date2: searchCreated)){
                                    testline.color = user.color ?? "#808080"
                                }else {
                                    testline.color = "#808080"
                                }
